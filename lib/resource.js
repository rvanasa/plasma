...
Resource.prototype = {
	...
	request: function(callback, errCallback)
	{
		...
		if(this.destroyed)
		{
			throw new Error('Resource is destroyed' + (this.id ? ': ' + this.id : ''));
		}
		else if(this.resolved || !this.provider)
		{
			// return callback ? callback(this.value) : this.value;
			return Promise.resolve(this.value).then(callback, errCallback);
		}
		
		if(!this.loading)
		{
			this.loading = Promise(function(resolve, reject)
			{
				resource.provider(function(value)
				{
		...
		if(callback)
		{
			this.loading.then(function(value)
			{
				callback(value);
				return value;
			});
		...
		if(errCallback)
		{
			this.loading.then(null, function(error)
			{
				callback(error);
				return error;
			});
...
// Resource.defer = function()
// {
// 	return new Resource(function(done) {done()});
// }
...
if(typeof module !== 'undefined') module.exports = Resource;